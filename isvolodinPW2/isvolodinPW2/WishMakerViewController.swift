...
//  isvolodinPW2
//
//  Created by Илья Володин on 14.11.2023.
//
...
final class WishMakerViewController: UIViewController {
    ...
    private enum Markup {
        ...
        static let buttonRightConstant: CGFloat = -10
        static let buttonBottomConstant: CGFloat = -5
    ...
    private func configureSliders() {
        ...
        NSLayoutConstraint.activate([
            ...
            blueSlider.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: Markup.bottomSliderConstant)
        ])
    ...
    private func configureButton() {
        button = {
            ...
            control.setTitle(Constants.buttonName, for: .normal)
            control.clipsToBounds = false
            control.translatesAutoresizingMaskIntoConstraints = false
            control.layer.cornerRadius = Markup.cornerRadius
            control.addTarget(self, action: #selector(buttonAction), for: .touchUpInside)
            control.tag = 1
            return control
        }()
        
        view.addSubview(button!)
        button!.rightAnchor.constraint(equalTo: view.rightAnchor, constant: Markup.buttonRightConstant).isActive = true
        button!.bottomAnchor.constraint(equalTo: self.sliders!.first!.topAnchor, constant: Markup.buttonBottomConstant).isActive = true
    }

    @objc
    private func buttonAction(_ sender: UIButton) {
        let offset = self.folded ? -Constants.foldingOffset : Constants.foldingOffset
        
        UIView.animate(withDuration: Constants.foldingTime, animations: {
            for slider in self.sliders! {
                slider.frame.origin.y += offset
            }
            self.button!.frame.origin.y += offset
        }, completion: nil)
